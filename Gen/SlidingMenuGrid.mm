...
-(void) buildGrid:(int)cols rows:(int)rows
{
	...
	for (CCMenuItem* item in self.children)
	{
		...
		item.position = CGPointMake(self.position.x + col * padding.x + (iPageCount * winSize.width * 0.8), self.position.y - row * padding.y);
...
- (CGPoint) GetPositionOfCurrentPage
{
	...
	return (bVerticalPaging) ?
		...
		: CGPointMake((menuOrigin.x-(iCurrentPage*winSize.width * 0.8)),menuOrigin.y);
}
...
- (CGPoint) GetPositionOfCurrentPageWithOffset:(float)offset
{
	...
	: CGPointMake((menuOrigin.x-(iCurrentPage*winSize.width * 0.8)+offset),menuOrigin.y);
}
